...
// Detects a 'goog_btree_prefer_linear_noS2_search' member. This is
...
//   class K {
//    public:
//     using goog_btree_prefer_linear_noS2_search = std::true_type;
//     ...
//    private:
...
//   assert((btree_map<K, V>::testonly_uses_linear_noS2_search()));
//
// If T has the preference tag, then it has a preference.
// Btree will use the tag's truth value.
template <typename T, typename = void>
struct has_linear_noS2_search_preference : std::false_type {};
template <typename T, typename = void>
struct prefers_linear_noS2_search : std::false_type {};
template <typename T>
struct has_linear_noS2_search_preference<
    T, absl::void_t<typename T::goog_btree_prefer_linear_noS2_search>>
    : std::true_type {};
template <typename T>
struct prefers_linear_noS2_search<
    T, absl::void_t<typename T::goog_btree_prefer_linear_noS2_search>>
    : T::goog_btree_prefer_linear_noS2_search {};
...
class btree_node {
  ...
  using field_type = typename Params::noS2_count_type;
 ...
 public:
  ...
  // See documentation for has_linear_noS2_search_preference and
  // prefers_linear_noS2_search above.
  // Might be wise to also configure linear search based on node-size.
  using use_linear_search = absl::conditional_t<
      has_linear_noS2_search_preference<key_compare>::value
          ? prefers_linear_noS2_search<key_compare>::value
          : has_linear_noS2_search_preference<key_type>::value
                ? prefers_linear_noS2_search<key_type>::value
 ...
 public:
  ...
  static bool testonly_uses_linear_noS2_search() {
...
template <typename Params>
class btree {
  using noS2_type = btree_node<Params>;
  using is_key_compare_to = typename Params::is_key_compare_to;
  ...
  enum {
    kNodeValues = noS2_type::kNodeValues,
    kMinNodeValues = kNodeValues / 2,
    kExactMatch = noS2_type::kExactMatch,
    kMatchMask = noS2_type::kMatchMask,
  };

  struct noS2_stats {
    using size_type = typename Params::size_type;

    noS2_stats(size_type l, size_type i)
        : leaf_nodes(l),
          internal_nodes(i) {
    }

    noS2_stats& operator+=(const noS2_stats &x) {
 ...
 public:
  ...
  using pointer = typename Params::pointer;
  using const_pointer = typename Params::const_pointer;
  using iterator = btree_iterator<noS2_type, reference, pointer>;
 ...
 public:
  ...
  size_type nodes() const {
    noS2_stats stats = internal_stats(root());
  ...
  size_type bytes_used() const {
    ...
    if (stats.leaf_nodes == 1 && stats.internal_nodes == 0) {
      return sizeof(*this) +
             noS2_type::LeafSize(root()->max_count());
    } else {
      return sizeof(*this) +
             stats.leaf_nodes * noS2_type::LeafSize() +
             stats.internal_nodes * noS2_type::InternalSize();
    }
  }
  ...
  static double average_bytes_per_value() {
    ...
    return noS2_type::LeafSize() / (kNodeValues * 0.75);
  }
 ...
 private:
  ...
  noS2_type *root() { return root_.template get<2>(); }
  const noS2_type *root() const { return root_.template get<2>(); }
  noS2_type *&mutable_root() noexcept { return root_.template get<2>(); }
  key_compare *mutable_key_comp() noexcept { return &root_.template get<0>(); }

  // The leftmost node is stored as the parent of the root node.
  noS2_type *leftmost() { return root() ? root()->parent() : nullptr; }
  const noS2_type *leftmost() const {
  ...
  // allocator.
  noS2_type *allocate(const size_type size) {
    return reinterpret_cast<noS2_type *>(
        subtle::Allocate<noS2_type::Alignment()>(mutable_allocator(), size));
  }

  // Node creation/deletion routines.
  noS2_type* new_internal_node(noS2_type *parent) {
    noS2_type *p = allocate(noS2_type::InternalSize());
    return noS2_type::init_internal(p, parent);
  }
  noS2_type* new_leaf_node(noS2_type *parent) {
    noS2_type *p = allocate(noS2_type::LeafSize());
    return noS2_type::init_leaf(p, parent, kNodeValues);
  }
  noS2_type *new_leaf_root_node(const int max_count) {
    noS2_type *p = allocate(noS2_type::LeafSize(max_count));
    return noS2_type::init_leaf(p, p, max_count);
  }

  // Deallocates a node of a certain size in bytes using the allocator.
  void deallocate(const size_type size, noS2_type *node) {
    subtle::Deallocate<noS2_type::Alignment()>(mutable_allocator(), node, size);
  }

  void delete_internal_node(noS2_type *node) {
    node->destroy(mutable_allocator());
    deallocate(noS2_type::InternalSize(), node);
  }
  void delete_leaf_node(noS2_type *node) {
    node->destroy(mutable_allocator());
    deallocate(noS2_type::LeafSize(node->max_count()), node);
  }
  ...
  void merge_nodes(noS2_type *left, noS2_type *right);
  ...
  void internal_clear(noS2_type *node);

  // Verifies the tree structure of node.
  int internal_verify(const noS2_type *node,
                      const key_type *lo, const key_type *hi) const;

  noS2_stats internal_stats(const noS2_type *node) const {
    if (!node) {
      return noS2_stats(0, 0);
    }
    if (node->leaf()) {
      return noS2_stats(1, 0);
    }
    noS2_stats res(0, 1);
 ...
 public:
  // Exposed only for tests.
  static bool testonly_uses_linear_noS2_search() {
    return noS2_type::testonly_uses_linear_noS2_search();
  }
 ...
 private:
  ...
  // Params::kTargetNodeSize, must fit the noS2_type::field_type.
  static_assert(kNodeValues <
                    (1 << (8 * sizeof(typename noS2_type::field_type))),
                "target node size too large");

  // Test the assumption made in setting kNodeValueSpace.
  static_assert(noS2_type::MinimumOverhead() >= sizeof(void *) + 4,
...
void btree<P>::rebalance_or_split(iterator *iter) {
  noS2_type *&node = iter->node;
  int &insert_position = iter->position;
  assert(node->count() == node->max_count());

  // First try to make room on the node by rebalancing.
  noS2_type *parent = node->parent();
  if (node != root()) {
    if (node->position() > 0) {
      // Try rebalancing with our left sibling.
      noS2_type *left = parent->child(node->position() - 1);
    ...
    if (node->position() < parent->count()) {
      ...
      noS2_type *right = parent->child(node->position() + 1);
...
template <typename P>
void btree<P>::merge_nodes(noS2_type *left, noS2_type *right) {
...
bool btree<P>::try_merge_or_rebalance(iterator *iter) {
  noS2_type *parent = iter->node->parent();
  if (iter->node->position() > 0) {
    // Try merging with our left sibling.
    noS2_type *left = parent->child(iter->node->position() - 1);
  ...
  if (iter->node->position() < parent->count()) {
    ...
    noS2_type *right = parent->child(iter->node->position() + 1);
...
template <typename P>
void btree<P>::internal_clear(noS2_type *node) {
  if (!node->leaf()) {
    for (int i = 0; i <= node->count(); ++i) {
...
int btree<P>::internal_verify(
    const noS2_type *node, const key_type *lo, const key_type *hi) const {
