...
DE_API S1Angle GetDistance(const S2Point& x, const S2Point& a, const S2Point& b);
...
DE_API bool IsDistanceLess(const S2Point& x, const S2Point& a, const S2Point& b,
                    S1ChordAngle limit);
...
DE_API bool UpdateMinDistance(const S2Point& x, const S2Point& a, const S2Point& b,
...
// The case A == B is handled correctly.
DE_API bool UpdateMaxDistance(const S2Point& x, const S2Point& a, const S2Point& b,
...
DE_API double GetUpdateMinDistanceMaxError(S1ChordAngle dist);
...
DE_API bool IsInteriorDistanceLess(const S2Point& x,
...
DE_API bool UpdateMinInteriorDistance(const S2Point& x,
                               const S2Point& a, const S2Point& b,
                               S1ChordAngle* min_dist);
...
DE_API S2Point Project(const S2Point& x, const S2Point& a, const S2Point& b,
...
DE_API double GetDistanceFraction(const S2Point& x,
                           const S2Point& a, const S2Point& b);
...
DE_API S2Point Interpolate(double t, const S2Point& a, const S2Point& b);
...
DE_API S2Point InterpolateAtDistance(S1Angle ax, const S2Point& a, const S2Point& b);
...
DE_API bool UpdateEdgePairMinDistance(const S2Point& a0, const S2Point& a1,
                               const S2Point& b0, const S2Point& b1,
                               S1ChordAngle* min_dist);

// As above, but for maximum distances.  If one edge crosses the antipodal
// reflection of the other, the distance is Pi.
DE_API bool UpdateEdgePairMaxDistance(const S2Point& a0, const S2Point& a1,
                               const S2Point& b0, const S2Point& b1,
                               S1ChordAngle* max_dist);
...
DE_API    std::pair<S2Point, S2Point> GetEdgePairClosestPoints(
...
DE_API bool IsEdgeBNearEdgeA(const S2Point& a0, const S2Point& a1,
                      const S2Point& b0, const S2Point& b1,
                      S1Angle tolerance);
...
DE_API inline bool IsDistanceLess(const S2Point& x, const S2Point& a,
                           const S2Point& b, S1ChordAngle limit) {
  return UpdateMinDistance(x, a, b, &limit);
}

DE_API inline bool IsInteriorDistanceLess(const S2Point& x, const S2Point& a,
                                   const S2Point& b, S1ChordAngle limit) {
