...
// out that it can be expressed as a "ss2" that can be computed
// independently when clipping the two endpoints A and B.  This function
// returns the ss2 for the given endpoint, which is an integer ranging from
// 0 to 3.  If the sum of the two ss2s is 3 or more, then AB does not
...
static int ClipDestination(
  ...
  *uv = scale_uv * GetExitPoint(scaled_n, GetExitAxis(scaled_n));
  ...
  // original endpoint B.
  //
  // We reject the segment if the sum of the ss2s of the two endpoints is 3
  ...
  if ((p - a).DotProd(a_tangent) < 0) {
    ss2 = 2;  // B' is on wrong side of A.
  } else if ((p - b).DotProd(b_tangent) < 0) {
    ss2 = 1;  // B' is on wrong side of B.
  }
  if (ss2 > 0) {  // B' is not in the interior of AB.
    if (b[2] <= 0) {
      ss2 = 3;    // B cannot be projected onto this face.
    } else {
      *uv = R2Point(b[0] / b[2], b[1] / b[2]);
    }
  }
...
bool ClipToPaddedFace(const S2Point& a_xyz, const S2Point& b_xyz, int face,
  ...
  // As described above, if the sum of the ss2s from clipping the two
  // endpoints is 3 or more, then the segment does not intersect this face.
  int a_ss2 = ClipDestination(b, a, -scaled_n, b_tangent, a_tangent,
                                scale_uv, a_uv);
  int b_ss2 = ClipDestination(a, b, scaled_n, a_tangent, b_tangent,
                                scale_uv, b_uv);
  return a_ss2 + b_ss2 < 3;
}
